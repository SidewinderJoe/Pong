...
namespace pong_csharp
{
    ...
    {
        bool goup; //boolean to be used to detect player position
        ...
        int cpuPoint = 0; //socre for the cpu
        ...
        {
            // this is the main timer event, this even will trigger every 20 milliseconds
            ...
            if (score < 5)
            {
                ...
                {
                    //then change the speed direction so it moves back up or down
                    speed = -speed;
                }
            }
            else
            {
                // if the soce is greater than 5
                // then make the game difficult by
                // allowing the CPU follow the ball so it does not miss
                cpu.Top = ball.Top + 30;
            }
            ...
            //if the ball either reaches the top of the screen or bottom
            ...
            {
                // then move player towards the top
                player.Top -= 8;
            }
            ...
            {
                // then move player towards the bottom of screen
                player.Top += 8;
            }

            //final score and ending the game
            // if the player scores more than 10 then we will stop the timer and show a message box
            if (score > 10)
            {
                gameTimer.Stop();
